...
【功能說明】
從 WhatsApp 提取訊息並儲存到本地電腦，支援文字和媒體檔案自動分類儲存，並自動複製媒體到網路位置。

一個獨立的 WhatsApp 訊息提取模組，可將訊息儲存到本地電腦資料夾，並自動複製媒體到指定的網路位置。
...
- ✅ **按日期整理**每天一個 JSON 檔案
- ✅ **媒體下載**自動下載圖片、影片、檔案
- ✅ **網路備份**自動複製媒體到 `V:\WhatsappBot\Media`
- ✅ **智能命名**媒體檔名使用 `日期_發訊息人名字_時間戳` 格式
- ✅ **自動回覆**複製完成後自動回覆確認
- ✅ **獨立運行**無需依賴其他模組
- ✅ **模組化設計**可被其他專案引用
- ✅ **v2.0** 支援每個聊天獨立儲存
...
媒體檔案會自動複製到 `V:\WhatsappBot\Media` 位置，檔名格式：

```
V:\WhatsappBot\Media\
├── 2026-04-02_JohnDoe_1712034000000.jpg
├── 2026-04-02_JaneSmith_1712034100000.jpg
└── ...
```

**檔名格式說明：**
- `2026-04-02` - 日期（YYYY-MM-DD）
- `JohnDoe` - 安全的發訊息人名字（特殊字符已替換）
- `1712034000000` - 時間戳（毫秒）
- `.jpg` - 原始副檔名
...
- **自動複製**媒體到 `V:\WhatsappBot\Media` 位置
- **智能命名**媒體檔名使用 `日期_發訊息人名字_時間戳` 格式
- **自動回覆**複製完成後自動回覆 `(已Save PIC to Server)`
- **安全處理**自動創建網路資料夾（若不存在）
- **錯誤處理**網路備份失敗不影響本地儲存
